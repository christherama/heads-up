<div class="seatingconfigs form">
<?php echo $this->Form->create('Seatingconfig');?>
	<fieldset>
		<legend><?php echo __('Edit Seatingconfig'); ?></legend>
	<?php
		echo $this->Form->input('seatingconfig_id');
		echo $this->Form->input('seatingconfig_name');
		echo $this->Form->input('seatingconfig_rows');
		echo $this->Form->input('seatingconfig_cols');
	?>
	</fieldset>
<?php echo $this->Form->end(__('Submit'));?>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>

		<li><?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $this->Form->value('Seatingconfig.seatingconfig_id')), null, __('Are you sure you want to delete # %s?', $this->Form->value('Seatingconfig.seatingconfig_id'))); ?></li>
		<li><?php echo $this->Html->link(__('List Seatingconfigs'), array('action' => 'index'));?></li>
	</ul>
</div>
